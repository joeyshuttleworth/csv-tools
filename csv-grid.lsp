(if (not *csv-tools-loaded*)
    (progn (princ "Please load csv-tools.lsp first\n")
	   (exit)))
(setq gap 2)
(setq *cell-types* (list NIL '(1 2) '(2 3) '(1 3)' (1 4) '(2 4) NIL '(3 4) '(3 4) NIL '(2 4) '(1 4) '(1 3) '(2 3) '(1 2) NIL)) ;code showing which sides of the square lines connect to, single numbers represent special cases 
(defun create-grid ( / fin c x y lst xlast ylast tmp xstart ystart step buf c)
(progn
 ...
 (while (setq threshold (getreal "\nEnter threshold for contour line or press enter to exit\n"))    ;first contour will take longer
   (if (not values)
...
(defun draw-contour (values step threshold / cell-grid i j start-edge end-edge spacing)
  (progn
    ...
    (repeat (- ystart ylast 1)
	    ...
	    (repeat (- xlast xstart 1)
		    ...
		    (if (or (= bitmask 6) (= bitmask 8))
		      (if (>= (mean (list (nmth i j values) (nmth i (+ 1 j) values )(nmth (+ 1 i) j values) (nmth (+ 1 i) (+ 1 j) values))) threshold)
			  (if (= bitmask 6)
			    (setq bitmask 8))
			(if (= bitmask 8)
			  (setq bitmask 6))))      
